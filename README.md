...
into the waste and creates a Profile "On" toolpath at the exact depth â€” the
...
one in particular â€” it draws vectors, offsets them and makes a Profile toolpath â€”
...
> Jobs built with the old version still work â€” see
...
1. Double-click `EdgeBreaker-v*.vgadget` to install, then restart Aspire or VCarve.
   ...
   it's new and not many people have it yet â€” choose Keep, then open it.
2. Open a job, select the closed vector(s) to chamfer.
3. Gadgets menu â†’ EdgeBreaker. The dialog opens with a coloured banner telling you
...
4. The gadget draws orange offset vectors and creates a calculated
   ...
   all â€” the toolpath in the Toolpaths panel and the orange offsets on the
...
restart Aspire or VCarve. It installs over the old copy â€” there's nothing to
uninstall first.
...
the name â€” that's how to tell what you're on.
...
of the dialog names it, in colour, and the OK button repeats it â€” so a run that
would destroy something says so before you press anything.

| What you have selected | Banner | What OK does |
|---|---|---|
| Shapes no chamfer uses | **Green â€” Adding Chamfer N** | Creates a new chamfer. Nothing existing is touched. |
| Shapes an existing chamfer was built from | **Blue â€” Rebuilding Chamfer K** | Rebuilds K, with K's own size, side and cut position already loaded. Selecting *some* of its shapes is enough. |
| Nothing | **Blue â€” Rebuilding Chamfer K, nothing selected** | Rebuilds K from the shapes it remembers. |
| Shapes a chamfer *should* know but doesn't | **Amber â€” I don't know which shapes** | Rebuilds it on your selection, and this run teaches it. |
| A chamfer you picked by hand, holding different shapes | **Red â€” Replacing Chamfer K's shapes** | Removes K's offsets and toolpath and rebuilds them on the new shapes. Cannot be undone. |

Two more things it works out on its own:

- **A mixed selection** â€” some free shapes, some already owned by another
  chamfer â€” adds a new chamfer from the free ones and says how many it left out.
...
**With nothing selected**, EdgeBreaker rebuilds the newest chamfer â€” unless you
...
the waste and cuts that one â€” and **`G` is how far out**. The dialog calls it the
standoff.
...
That's what the **0%â€“100%** buttons pick. **0%** is the smallest gap â€” the cut sits
low, just clear of the tip. **100%** is the largest â€” up near the top of the flute.
...
chamfer on a small bit gets refused â€” the chamfer eats into the top of the range,
and past a certain size there's nothing left in between.

## Start depth

**Start depth is how far below the top of the stock the edge sits** â€” a pocket
...
wherever the edge happens to sit â€” a start depth simply moves the whole cut down.
...
settings that built them â€” size, mode, side, cut position, start depth and units.
That is what
...
- Delete a chamfer's toolpath and it is forgotten â€” which is also how you free
...
V-bit that cuts it, and the four numbers that describe it â€” `W` the chamfer
...
On any bit that isn't 90Â°, a small corner detail sits at the right-hand end of the
drawing. It's the corner blown up, with the edge you're sizing picked out in orange
and named underneath â€” `SETBACK` across the top face, `FACE` along the slant, `LEG`
down the side â€” and the number you typed below that.
...
A 90Â° bit doesn't get one â€” at 90Â° the setback and the leg are the same measurement,
so there's no choice to explain.

## More than one chamfer in a job

A job can hold up to 99 chamfers, each with its own bit, size and cut position â€”
a fine chamfer on the lettering and a heavier one on the outline, say.

Each chamfer keeps its own offset layer (`EdgeBreaker - Offset 01`, `02`, â€¦) and
...
their size, but not their shapes â€” nothing recorded them at the time â€” so the first
...
that it found one and then leaves it alone entirely â€” never listed, never replaced,
...
are used exactly as the database has them** â€” so a chamfer is cut with the same
...
hand over a tool it has no cutting data for â€” not a gadget fault.
...
this gadget wrote itself. The bit inside it is irrelevant â€” it is swapped for
the one you picked, and so is the depth.

So is the layer: building Chamfer 3 rewrites the restriction to
`EdgeBreaker - Offset 03` before the template is loaded â€” a two-character
change, nothing else in the file moves â€” and the gadget reads the name back out
...
restricted to **`EdgeBreaker - Offset 01`** â€” the `01` matters, because that is
...
- The layers `EdgeBreaker - Offset 01`, `02`, â€¦ are gadget-owned. Building a
  chamfer clears **its own** layer and no other, and gadget deletions cannot be
  undone â€” never keep your own work on one.
...
- A feature too narrow to chamfer at the size you asked for is **skipped**, not
  ...
  orange offset beside it. A skip is one of the things that breaks the silence â€”
...
- One vector can produce **more than one** offset vector. Where an inward offset
  ...
  pieces come back as separate closed loops â€” so the drawn count need not match
  what you selected.
- The gadget owns every toolpath whose name contains `[EdgeBreaker NN]`, and a
  ...
  **Toolpath deletions cannot be undone.** To keep a chamfer toolpath forever,
  rename it so the marker is gone â€” the gadget will never touch it again, and it
...
- The gadget calculates its own toolpath and leaves your other toolpaths
  ...
  so â€” then open the toolpath and click Calculate yourself.
- A chamfer with no memory of its own falls back to your **last run's** entries,
  ...
  back to the default â€” a size from an inch job when the open job is metric.
